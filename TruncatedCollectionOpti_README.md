# `TruncatedCollectionOfTOpt<T>`

Here is the complete code for the `TruncatedCollectionOfTOpt<T>` class, which implements a truncated collection with optional total count and page size.

This is the optimized version that includes constructors for backward compatibility, static creation methods, and async support.
